...
# Datos
ventas = pd.DataFrame(get(db.ventasTable))
detalles = pd.DataFrame(get(db.detalleVentaTable))
productos = pd.DataFrame(get(db.productoTable))
formas_pago = pd.DataFrame(get(db.formaPagoTabla))
...
if not ventas.empty:
    ...
    ventaCol, producto, ventasTotal = st.columns(3)
